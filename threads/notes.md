...
Тут самая простая реализация с кооперативной многозадачностью. Есть только создание потока и планировщик; join, cancel, очистка ресурсов не реализованы.
Поскольку планированием занимается пользователь, а не ядро, то нам надо самостоятельно управлять контекстом исполнения. В контекст так же входит стек. Контекст каждого потока будем сохранять в его управляющей структуре.

### Создание потока
Для создания используем функцию `uthread_create()`. Сначала создаем стек так же, в предыдущем пункте. Управляющую структуру будем хранить в последней странице выделенного региона.
Далее создаем контекст данного потока. Для представления контекста есть структура `ucontext_t`. Последовательность действий такая:
1. Прочитать текущий контекст при помощи `getcontext()`;
2. Настроить нужные поля в полученной структуре;
3. Вызвать `makecontext()`, передав туда полученную структуру, чтобы корректно задать контекст.
В структуре `ucontext_t` есть три поля, предназначенные для нашего использования. Это uc_link, uc_sigmask, uc_stack.
uc_link указывает на контекст, который будет запущен после того, как данный контекст завершит исполнение. Указываем NULL, это значит, что следующий контекст запущен не будет, а поток (как ядерная единица планирования) просто завершится.
uc_sigmask - маска блокируемых сигналов, не изменяем.
uc_stack - стек, представленный структурой `stack_t`. Там нас интересует ss_size - размер стека, и ss_sp - это начало региона, выделенного под стек. **Важно:** это именно начало региона, а не начало стека.
Затем заполняем оставшиеся поля управляющей структуры. К этому момементу новый user-space поток создан, но еще не запущен. Запуском потоков занимается наш планировщик.

### Планировщик
В варианте с кооперативной многозадачностью каждый поток должен явно вызывать функцию планировщика, чтобы передать управление другой задаче. Функция планировщика сохраняет текущий контекст данного потока, чтобы потом его можно было запустить с того же места. Затем загружается контекст другого потока, и он начинает исполняться. Эти два действия выполняются функцией `swapcontext()`.
